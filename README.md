...
**中国象棋 3D 对战**：固定斜俯视、人偶棋子、走子动画、吃子战斗特效、战果 HUD。
...
> `resources/reference/` 下约 1 GB 的源视频、逐帧序列与裁切素材仅在本地留存，已由
> `.gitignore` 排除；远端克隆会保留说明文档和预览入口，但不会包含这些大体积参考素材。
...
- `M`：选择本地双人或人机对弈；人机模式固定玩家执红、电脑执黑，可选入门 / 标准 / 挑战。
...
HUD 工具栏上的 **「演示 D」** 按钮（或直接按 `D`）会跑一段约 **55–58 秒**的自动对弈：
开场环绕、炮的弹道、车的冲锋拖尾、马的日字腾跃、边打边转的镜头，最后落在绝杀上。
再点一次按钮、按 `D` 或 `Esc` 可随时停止。

录屏前：

- 确认是**本地双人**模式（按 `M`）——人机模式下电脑对手会和演示抢着走棋，演示会拒绝启动并说明原因。
- 想要满屏就按 `F`；想要纯画面无 HUD，把 `src/demo/demoReel.ts` 里 `runDemoReel` 的
  `cinema` 传 `true`（`main.ts` 调用处）。
- 点按钮这一下本身就是真实用户手势，**音效会自动解锁**，不必额外点别处。

倒数 3 秒后开始，控制台逐手打印，跑完给出实际秒数。想卡准 60 秒，
按 `pace = 60 / 实际秒数` 传给 `runDemoReel`。

几个实现上的选择：

- **棋盘输入走真实通道**：落子与转视角都是往画布派发 `PointerEvent`，和真人操作
  同一条路径，因此拾取、输入锁、演出时间线、音效派发全都照常发生——录下来的
  就是产品真实行为，没有“演示专用捷径”。重开、录制模式这类应用级命令则由
  `main.ts` 以回调传入，因为演示期间键盘是锁着的（免得中途悔棋把局面走岔）。
- **按需加载**：模块只在点击时 `import()`，生产构建里是独立 chunk（gzip 约 2.6 kB），
  不点就不下载。
- **对局线由引擎离线搜出**，并由 `src/engine/demoReel.test.ts` **直接读源文件**
  持续校验：规则一旦改动使某一手失效，测试立刻失败，而不是等到真去录片子时才发现。
...
- TypeScript + Vite
- Three.js（3D 场景）
- Web Worker 确定性电脑对手（固定节点预算、三级难度）
- 规则引擎与渲染分离
...
6. **Phase 5 ✅**：本地双人 / 人机模式、三级电脑对手、Worker 失效保护与整回合悔棋
7. **Phase 6 ✅**：程序竞赛循环规则、自然限着、死局裁定、规则感知电脑搜索与统一中文提示
